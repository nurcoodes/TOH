...
def value_iteration(
    ...
    for state in mdp.nonterminal_states:
        max_value_for_state = float('-inf')
        for action in mdp.actions:  # Ensure this iterates over all applicable actions for the state
            # Calculate Q-value for each action
            q_value = sum(mdp.transition(state, action, next_state) * (mdp.reward(state, action, next_state) + mdp.config.gamma * v_table[next_state]) for next_state in mdp.all_states)
            q_table[(state, action)] = q_value
            max_value_for_state = max(max_value_for_state, q_value)
        
        # Update the value table for the state with the maximum Q-value found
        new_v_table[state] = max_value_for_state
        
        # Calculate max_delta
        delta = abs(v_table.get(state, 0) - max_value_for_state)
...
def extract_policy(
    ...
    for state in mdp.nonterminal_states:
        ...
        for action in [op.name for op in mdp.operators if op.pre_condition(state)] + ['Exit']:
...
def q_update(
    ...
    # *** BEGIN OF YOUR CODE ***
    future_reward = max(q_table.get((next_state, next_action), 0) for next_action in mdp.actions)
    q_table[(state, action)] = q_table.get((state, action), 0) + alpha * (reward + mdp.config.gamma * future_reward - q_table.get((state, action), 0))
...
def extract_v_table(mdp: tm.TohMdp, q_table: tm.QTable) -> tm.VTable:
    ...
    """
    # *** BEGIN OF YOUR CODE ***
    return {state: max(q_table.get((state, action), float('-inf')) for action in mdp.actions) for state in mdp.all_states}
...
def choose_next_action(
    ...
    next_action = epsilon_greedy(best_actions, epsilon)
    return next_action
...
def custom_epsilon(n_step: int) -> float:
    ...
    """
    # *** BEGIN OF YOUR CODE ***
    return max(0.01, 1 * (1 - 0.001) * n_step)
